...
import type { Coupon, CouponWithStats, MyCoupon } from '../types/coupon';

const TABLE_COUPONS = 'ah_coupons';
const TABLE_USES = 'ah_coupon_uses';
const CODE_PREFIX = 'CPL';

/** 쿠폰 코드 자동 생성: CPL-20260415-K3M7 */
...
export async function listCoupons(): Promise<CouponWithStats[]> {
  ...
  const couponIds = (coupons || []).map((c: Coupon) => c.id);
...
export async function redeemCoupon(
  ...
  try {
    await supabase.rpc('grant_coupon_license', {
      p_user_id: userId,
