const studioOverview = {
  ...
  titleEn: 'Copilot Studio',
  sections: [
    {
      title: 'Copilot Studio란?',
      titleEn: 'What is Copilot Studio?',
      content: `**Microsoft Copilot Studio**는 Power Virtual Agents의 진화형으로, 조직 맞춤형 AI Copilot을 **로우코드(Low-Code)** 방식으로 빌드할 수 있는 플랫폼입니다. 코딩 경험이 없어도 자연어 기반의 대화형 AI 에이전트를 만들고 배포할 수 있습니다.
...
> Copilot Studio는 copilotstudio.microsoft.com에서 무료 체험이 가능합니다. Microsoft 365 계정이 있다면 바로 시작할 수 있으며, 별도의 개발 환경 설정이 필요하지 않습니다.`,
      contentEn: `**Microsoft Copilot Studio** is the evolution of Power Virtual Agents, a platform that lets you build organization-specific AI Copilots using a **Low-Code** approach. You can create and deploy conversational AI agents based on natural language without any coding experience.
...
> Copilot Studio offers a free trial at copilotstudio.microsoft.com. If you have a Microsoft 365 account, you can start immediately with no additional development environment setup required.`
    },
    {
      title: '커스텀 Copilot 만들기',
      titleEn: 'Building Custom Copilots',
      content: `Copilot Studio에서 커스텀 Copilot을 만드는 과정은 **직관적이고 체계적**입니다. 주제(Topics) 설정부터 대화 흐름 디자인, 지식 소스 연결까지 단계별로 살펴봅니다.
...
> 처음 Copilot을 만들 때는 3-5개의 핵심 주제부터 시작하세요. 사용자 피드백을 수집하며 점차 주제를 확장하는 것이 가장 효과적인 접근 방법입니다.`,
      contentEn: `Building a custom Copilot in Copilot Studio is an **intuitive and systematic** process. Let's walk through each step from setting up Topics to designing conversation flows and connecting knowledge sources.
...
> When building your first Copilot, start with 3-5 core topics. The most effective approach is to collect user feedback and gradually expand topics over time.`
    },
    {
      title: '플러그인과 커넥터',
      titleEn: 'Plugins & Connectors',
      content: `Copilot Studio의 **플러그인과 커넥터**를 활용하면 외부 시스템과 서비스를 연동하여 Copilot의 기능을 확장할 수 있습니다. Power Platform 생태계와의 통합이 핵심 강점입니다.
...
> 플러그인 개발 시 Microsoft의 AI Plugin 표준을 따르면 Copilot Studio뿐만 아니라 Microsoft 365 Copilot, ChatGPT 등 여러 플랫폼에서 재사용할 수 있습니다.`,
      contentEn: `By leveraging **plugins and connectors** in Copilot Studio, you can extend your Copilot's capabilities by integrating with external systems and services. Integration with the Power Platform ecosystem is a core strength.
...
> When developing plugins, following Microsoft's AI Plugin standard allows reuse not only in Copilot Studio but also across platforms like Microsoft 365 Copilot and ChatGPT.`
    },
    {
      title: '배포 및 분석',
      titleEn: 'Deployment & Analytics',
      content: `커스텀 Copilot을 완성한 후에는 **다양한 채널에 배포**하고, 분석 대시보드를 통해 성과를 모니터링하며 지속적으로 개선해야 합니다.
...
> 배포 후 최소 2주간의 데이터를 수집한 뒤 본격적인 최적화를 시작하세요. 초기 데이터는 패턴을 파악하기에 부족할 수 있으므로, 충분한 데이터가 쌓인 후 의미 있는 개선을 진행하는 것이 효과적입니다.`,
      contentEn: `After completing your custom Copilot, you need to **deploy it across various channels**, monitor performance through analytics dashboards, and continuously improve.
...
> Start serious optimization after collecting at least 2 weeks of post-deployment data. Initial data may be insufficient for pattern recognition, so meaningful improvements should begin after sufficient data has accumulated.`
    }
  ]
};
