const powerOverview = {
  ...
  titleEn: 'Power Platform Copilot',
  sections: [
    {
      title: 'Power Platform Copilot이란?',
      titleEn: 'What is Power Platform Copilot?',
      content: `Microsoft Power Platform은 **로우코드/노코드** 기반의 비즈니스 애플리케이션 플랫폼으로, Copilot AI가 통합되어 자연어만으로 업무 자동화, 앱 개발, 데이터 분석, 웹 사이트 구축이 가능합니다.
...
- **모니터링**: Power Platform 관리 센터에서 사용량, 성능, 오류 추적`,
      contentEn: `Microsoft Power Platform is a **low-code/no-code** business application platform with integrated Copilot AI, enabling workflow automation, app development, data analysis, and website building using just natural language.
...
- **Monitoring**: Track usage, performance, and errors through the Power Platform Admin Center`
    },
    {
      title: 'Power Automate + Copilot',
      titleEn: 'Power Automate + Copilot',
      content: `Power Automate의 Copilot을 사용하면 **자연어 한 줄**로 복잡한 업무 워크플로우를 자동 생성할 수 있습니다. 프로그래밍 지식 없이도 기업의 반복 업무를 혁신적으로 자동화합니다.
...
> 승인 흐름에서 **타임아웃 설정**을 반드시 추가하세요. 매니저가 응답하지 않을 경우를 대비해 3일 후 자동 리마인더, 5일 후 상위 매니저로 에스컬레이션하는 로직을 넣으면 프로세스가 멈추지 않습니다.`,
      contentEn: `With Power Automate's Copilot, you can automatically generate complex business workflows with **just one line of natural language**. Innovatively automate repetitive enterprise tasks without any programming knowledge.
...
> Always add a **timeout setting** to approval flows. To handle cases where the manager doesn't respond, add auto-reminder after 3 days and escalation to upper management after 5 days to prevent the process from stalling.`
    },
    {
      title: 'Power Apps + Copilot',
      titleEn: 'Power Apps + Copilot',
      content: `Power Apps의 Copilot을 사용하면 **자연어 설명만으로** 완전한 비즈니스 애플리케이션을 생성할 수 있습니다. 데이터 테이블, UI 화면, 비즈니스 로직까지 AI가 자동으로 구성합니다.
...
> 앱을 배포하기 전에 반드시 **다양한 디바이스에서 테스트**하세요. 특히 모바일에서 입력 폼의 사용성을 확인하고, 네트워크가 불안정한 환경(현장/창고)에서도 오프라인 기능이 동작하는지 점검해야 합니다.`,
      contentEn: `With Power Apps Copilot, you can create complete business applications **using only natural language descriptions**. AI automatically configures data tables, UI screens, and business logic.
...
> Before deploying your app, always **test on various devices**. Especially verify input form usability on mobile, and check that offline functionality works in environments with unstable networks (field/warehouse).`
    },
    {
      title: 'Power BI + Copilot',
      titleEn: 'Power BI + Copilot',
      content: `Power BI의 Copilot은 **자연어로 데이터를 질문**하면 즉시 시각화와 인사이트를 생성합니다. 복잡한 DAX 수식도 대화형으로 작성하고, 보고서를 자동으로 요약할 수 있습니다.
...
> Power BI Copilot을 사용할 때 **데이터 모델의 품질이 결과의 품질을 결정**합니다. 올바른 관계 설정, 명확한 테이블/컬럼 이름, 적절한 데이터 타입 설정이 선행되어야 Copilot이 정확한 인사이트를 제공합니다.`,
      contentEn: `Power BI's Copilot instantly generates visualizations and insights when you **ask questions about data in natural language**. You can also write complex DAX formulas conversationally and auto-summarize reports.
...
> When using Power BI Copilot, **data model quality determines result quality**. Proper relationship setup, clear table/column naming, and appropriate data type configuration must come first for Copilot to provide accurate insights.`
    }
  ]
};
