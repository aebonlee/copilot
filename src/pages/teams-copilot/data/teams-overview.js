export default {
  ...
  title: 'Teams Copilot',
  titleEn: 'Teams Copilot',
  sections: [
    {
      title: 'Teams Copilot이란?',
      titleEn: 'What is Teams Copilot?',
      content: `Teams Copilot은 Microsoft Teams에 통합된 AI 비서로, 회의, 채팅, 채널 활동을 지능적으로 지원합니다. 회의 내용을 실시간으로 요약하고, 놓친 채팅을 정리하며, 팀 협업을 한 단계 끌어올립니다.
...
> Teams Copilot의 회의 기능을 사용하려면 반드시 회의 녹음(Recording)과 전사(Transcription)가 활성화되어 있어야 합니다. IT 관리자에게 이 설정이 켜져 있는지 확인하세요.`,
      contentEn: `Teams Copilot is an AI assistant integrated into Microsoft Teams that intelligently supports meetings, chats, and channel activities. It summarizes meeting content in real-time, organizes missed chats, and elevates team collaboration to the next level.
...
> To use Teams Copilot's meeting features, meeting Recording and Transcription must be enabled. Check with your IT admin to confirm these settings are turned on.`,
    },
    {
      title: '회의에서의 Copilot',
      titleEn: 'Copilot in Meetings',
      content: `Teams 회의에서의 Copilot은 회의 생산성을 극대화하는 핵심 도구입니다. 회의 중 실시간으로 내용을 요약하고, 종료 후에는 회의록과 액션 아이템을 자동으로 생성합니다.
...
> 회의 Copilot은 녹음이 시작된 이후의 내용만 분석할 수 있습니다. 회의 시작과 동시에 녹음을 시작하는 것이 중요합니다. 또한, 회의 녹음에 대한 참석자 동의를 반드시 구하세요.`,
      contentEn: `Copilot in Teams meetings is the essential tool for maximizing meeting productivity. It summarizes content in real-time during meetings, and automatically generates meeting notes and action items after the meeting ends.
...
> Meeting Copilot can only analyze content after recording has started. It's critical to start recording at the beginning of the meeting. Also, always obtain attendee consent for meeting recording.`,
    ...
    {
      title: '채팅에서의 Copilot',
      titleEn: 'Copilot in Chat',
      content: `Teams 채팅에서의 Copilot은 대화의 흐름을 놓치지 않도록 도와줍니다. 긴 채팅 스레드를 빠르게 요약하고, 핵심 포인트를 추출하며, 적절한 답변을 작성하는 데 도움을 줍니다.
...
> 채팅 Copilot은 최근 30일 이내의 채팅 내용만 분석할 수 있습니다. 더 오래된 내용을 찾으려면 Teams 검색 기능을 활용하세요.`,
      contentEn: `Copilot in Teams chat helps you stay on top of conversations. It quickly summarizes long chat threads, extracts key points, and helps compose appropriate replies.
...
> Chat Copilot can only analyze chat content from the last 30 days. Use the Teams search function for older content.`,
    },
    {
      title: '채널과 협업',
      titleEn: 'Channels & Collaboration',
      content: `Teams 채널에서의 Copilot은 팀 전체의 협업을 강화합니다. 채널의 주요 활동을 요약하고, 문서 협업을 지원하며, 작업을 효율적으로 관리할 수 있게 돕습니다.
...
> 채널 Copilot은 공개 채널과 비공개 채널 모두에서 동작합니다. 단, 사용자는 해당 채널에 접근 권한이 있어야 Copilot을 통해 내용을 확인할 수 있습니다. 보안 정책에 따라 권한이 없는 채널의 정보는 제공되지 않습니다.`,
      contentEn: `Copilot in Teams channels strengthens collaboration across the entire team. It summarizes key channel activity, supports document collaboration, and helps manage tasks efficiently.
...
> Channel Copilot works in both public and private channels. However, users must have access to the channel to view content through Copilot. Information from channels you don't have permission to access will not be provided, in accordance with security policies.`,
    },
  ],
};
